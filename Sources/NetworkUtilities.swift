...
struct NetworkUtilities {
        ...
        while true {
            ...
            if (flags & IFF_UP) == IFF_UP && (flags & IFF_LOOPBACK) == 0 {
                if addr.sa_family == UInt8(AF_INET) {
                    ...
                    let address = String(cString: inet_ntoa(sin.sin_addr))
                    if isRoutableIPv4(sin.sin_addr) && !address.isEmpty {
                    ...
                    if let ipPtr {
                        let address = String(cString: ipPtr)
                        if isRoutableIPv6(sin6.sin6_addr) && !address.isEmpty {
                            results.append((name, address))
                        }
                    }
                }
    ...
    private static func isRoutableIPv6(_ addr: in6_addr) -> Bool {
        let addrBytes = Mirror(reflecting: addr.__u6_addr.__u6_addr8).children.compactMap { $0.value as? UInt8 }
    ...
    static func defaultInterface() -> String? {
        ...
        for line in output.components(separatedBy: "\n") where line.starts(with: "default") {
